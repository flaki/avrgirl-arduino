...
Avr109.prototype._reset = function(callback) {
  ...
  _this.connection.serialPort.once('close', function(dc) {
    console.log('dc', dc)
    async.series([
  ...
  async.series([
    ...
    tempSerialPort.close.bind(tempSerialPort)
    // it seems that on latest node-serialport versions
    // the board reset does not automatically close the connection
    // so the actual flashing never starts
  ],
  function(error) {
    if (error) {
      return callback(error);
